# 👋 Hi, I'm Muhammad Ijaz

**FPGA TEAM LEAD | Verilog · VHDL | RF/DSP Systems | Embedded Linux**

I work at the intersection of algorithm design and hardware reality, taking designs from RTL through timing closure to silicon running correctly under real-world conditions. Over 7 years across the full FPGA development cycle on Xilinx and Intel platforms: RTL design, digital logic architecture, DSP algorithm implementation, high-speed AXI-Stream pipelines, and RF transceiver integration with production platforms including the AD9361, AD9364, ADRV9002, and ADRV9009.

I work across the full stack, from HDL through high-speed data interfaces into embedded Linux, and I'm as comfortable on the bench with a spectrum analyzer and oscilloscope as I am at the RTL simulator, validating that designs behave as intended under real conditions.

I contribute a technical blog series on FPGA-based design and real-time signal processing on Analog Devices' EngineerZone.

📬 Open to FPGA/RTL engineering roles across any industry. Always happy to connect.

- 🔭 **Currently working on:** Radar/RF control firmware and FPGA logic on the ZCU102 platform
- 🌱 **Currently learning:** AI on FPGA
- 👯 **Looking to collaborate on:** Open-source FPGA/RTL projects, HDL verification tooling, or embedded Linux on FPGA SoCs
- 🤔 **Looking for help with:** PCIe (PCI-Express) practical implementation on FPGA
- 💬 **Ask me about:** Verilog/VHDL design & verification, Vivado/Vitis, Quartus Prime, RF/DSP pipelines, embedded Linux on FPGA SoCs
- 📫 **Reach me:** ijazmuhammad898@gmail.com

## 🌐 Find me here

[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white)](https://linkedin.com/in/muhammad-ijaz-swarmlab)
[![YouTube](https://img.shields.io/badge/YouTube-%23FF0000.svg?logo=YouTube&logoColor=white)](https://www.youtube.com/@TechXORT)
[![Technical Blogs](https://img.shields.io/badge/Technical_Blogs-2EA44F?style=for-the-badge&logo=readme&logoColor=white)](https://github.com/Ijaz-Muhammad/technical-blogs)
[![Email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:ijazmuhammad898@gmail.com)
## 🛠️ Tech Stack

**HDL & FPGA Design**
![Verilog](https://img.shields.io/badge/Verilog-CC0000?style=for-the-badge&logo=v&logoColor=white)
![VHDL](https://img.shields.io/badge/VHDL-004488?style=for-the-badge)
![Xilinx Vivado](https://img.shields.io/badge/Vivado-E31937?style=for-the-badge)
![Xilinx ISE](https://img.shields.io/badge/Xilinx%20ISE-E31937?style=for-the-badge)
![Intel Quartus](https://img.shields.io/badge/Quartus%20Prime-0071C5?style=for-the-badge)
![Vitis HLS](https://img.shields.io/badge/Vitis%20HLS-E31937?style=for-the-badge)
![ModelSim](https://img.shields.io/badge/ModelSim-333333?style=for-the-badge)

**Embedded & Firmware**
![C](https://img.shields.io/badge/c-%2300599C.svg?style=for-the-badge&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/c++-%2300599C.svg?style=for-the-badge&logo=c%2B%2B&logoColor=white)
![Embedded Linux](https://img.shields.io/badge/Embedded%20Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Linux Kernel](https://img.shields.io/badge/Linux%20Kernel-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Qt Creator](https://img.shields.io/badge/Qt%20Creator-41CD52?style=for-the-badge&logo=qt&logoColor=white)

**Analysis & Scripting**
![MATLAB](https://img.shields.io/badge/MATLAB-0076A8?style=for-the-badge&logo=mathworks&logoColor=white)
![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![TCL](https://img.shields.io/badge/TCL-3E7A9E?style=for-the-badge)

**Version Control & Platforms**
![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

## ⚙️ FPGA & Systems Skills

**Interfaces & Protocols:** AXI / AXI-Stream · SPI · I2C · UART · DMA · DDR3 SDRAM · HDMI · 10G Ethernet · SFP+ · TCP/UDP

**Debug & Verification:** Vivado ILA · VIO · ModelSim · Timing Analysis · Clock Domain Crossing (CDC) · Hardware Security

**DSP & RF:** SDR · RF SoC · FFT/IFFT · Fixed-Point Implementation · Digital Signal Processing · High-Level Synthesis (HLS)

**Embedded Systems:** MicroBlaze Softcore Processor · RTOS · Device Tree Customization · Kuiper Linux · libiio

**Other:** Stochastic Computing · PCB Design · Version Control (Git/GitHub)
## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Ijaz-Muhammad&show_icons=true&theme=github_dark&hide_border=true" height="180"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Ijaz-Muhammad&layout=compact&theme=github_dark&hide_border=true" height="180"/>
</p>
